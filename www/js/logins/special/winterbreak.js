...
//			candles[i].style.backgroundColor = "cyan";
		...
		candleParticle.src = i2root+"www/pics/flame_particles.gif"; // ---- get the particle animation
...
//	document.getElementsByClassName("middle")[0].style.paddingTop = "0px";
	...
	clockContainer.style.marginTop = "-75px";
	...
	clock.style.paddingBottom = "15px";
...
function incrementCountdown() {
	...
	if(parseInt(day) == 0 && parseInt(hr) == 0 && parseInt(min) == 1) {
		...
		clock.innerHTML = "<span style=\"font-size:" + (320 - Math.round(200 * scaleFactor)) + "px\">" + (day > 0 ? day+':':'') + hr + ":" + min + ":</span>" + sec;
		//if (Math.round(diff / 1000.0) == 0 && !snowing) {
			//initsnow();
		//}
	} else {
		clock.innerHTML = (day > 0 ? day+':':'') + hr + ":" + min + ":" + sec;
