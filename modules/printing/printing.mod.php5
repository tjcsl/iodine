...
class Printing {
	...
	public static function print_parking($people, $format = 'pdf') {
		$output = self::latexify('parking','parking');
		ob_start();
		eval($output);
		$output = ob_get_clean();
		if($format == 'print') {
	...
	* @param $module string The module to get the printing_path from
	*/
	static function latexify($filename, $module) {
		if(!self::$printing_path) {
			self::$printing_path = i2config_get('printing_path', NULL, $module);;
		}
