	...
	public static function get_new_message() {
		...
		try {
			if($fgetc = News::curl_file_get_contents($url)) {
                ...
                $snowdayd = preg_replace('<!--(.*?)-->', '', $snowdayd); // HTML comments
				...
				if(!empty($d[0]) && !empty($dn[0])) {
					...
					if(trim($ddate[0]) == "Monday, March 25") {
						...
						$einfo= "<!-- ".print_r($d,1).print_r($dn,1)." -->".
								"<div class='emerg' style='background-color:yellow;padding:5px'><span style='font-size:22px'>FCPS Emergency Information:</span> &nbsp; &nbsp; <a href='".$url."'>View details</a> &nbsp; <a href='?&get_new_message'>Refresh</a> &nbsp; <a href='#' onclick='$(\".emerg\").hide()'>Close</a><p>".trim($dn[0])."</p></div>";
					}
