	...
	*/
	public function __construct($activityid, $blockid = NULL) {
		...
		if ($activityid != NULL && $activityid != "") {
			...
			if($blockid) {
				$this->data = array_merge($this->data,$I2_SQL->query("SELECT bid,sponsors AS block_sponsors,rooms AS block_rooms,cancelled,comment,advertisement,attendancetaken FROM eighth_block_map WHERE bid=%d AND activityid=%d", $blockid, $activityid)->fetch_array(MYSQL_ASSOC));
