...
class EighthPrint {
	...
	*/
	public static function print_class_roster($aid, $bid, $format = 'print') {
		...
		$output = Printing::latexify('class_roster','eighth');
		ob_start();
		eval($output);
		$output = ob_get_clean();
		if($format == 'print') {
			...
			if($format == 'pdf') {
				Printing::add_info($output, self::$author, 'Print Class Roster', "{$activity->name} ({$activity->block->date} - {$activity->block->block} Block)", self::$creator);
			}
	...
	*/
	public static function print_sponsor_schedule($sid, $format = 'print') {
		...
		$output = Printing::latexify('sponsor_schedule','eighth');
		ob_start();
		eval($output);
		$output = ob_get_clean();
		if($format == 'print') {
			...
			if($format == 'pdf') {
				Printing::add_info($output, self::$author, 'Print Sponsor Schedule', $sponsor->name, self::$creator);
			}
	...
	public static function print_activity_schedule($aid, $format = 'print') {
		...
		$output = Printing::latexify('activity_schedule','eighth');
		ob_start();
		eval($output);
		$output = ob_get_clean();
		if($format == 'print') {
			...
			if($format == 'pdf') {
				Printing::add_info($output, self::$author, 'Print Activity Schedule', $activity->name, self::$creator);
			}
	...
	*/
	public static function print_attendance_data($aid, $bid, $format = 'print') {
		$activity = new EighthActivity($aid, $bid);
		$output = Printing::latexify('attendance_data','eighth');
		ob_start();
		eval($output);
		$output = ob_get_clean();
		if($format == 'print') {
			...
			if($format == 'pdf') {
				Printing::add_info($output, self::$author, 'Print Attendance Data', "{$activity->name} ({$activity->block->date} - {$activity->block->block} Block)", self::$creator);
			}
	...
	*/
	public static function print_activity_rosters($bids, $color, $format = "print") {
		...
		$output = Printing::latexify('activity_rosters','eighth');
		ob_start();
		eval($output);
		$output = ob_get_clean();
		if($format == 'print') {
			...
			if($format == 'pdf') {
				Printing::add_info($output, self::$author, 'Print Activity Rosters', implode(', ', $blocks) . ' Activity Rosters', self::$creator);
			}
	...
	*/
	public static function print_student_schedule($uid, $start_date = NULL, $format = 'html') {
		...
		$output = Printing::latexify('student_schedule','eighth');
		ob_start();
		eval($output);
		$output = ob_get_clean();
		if($format == "print") {
			...
			if($format == "pdf") {
				Printing::add_info($output, self::$author, 'Print Student Schedule', $user->name, self::$creator);
			}
	...
	*/
	public static function print_room_utilization($bid, $format = 'pdf') {
		...
		$output = Printing::latexify('room_utilization','eighth');
		ob_start();
		eval($output);
		$output = ob_get_clean();
		if($format == 'print') {
			...
			if($format == 'pdf') {
				Printing::add_info($output, self::$author, 'Print Room Utilization', "{$activity->block->date} - {$activity->block->block} Block", self::$creator);
			}
