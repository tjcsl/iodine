...
class Prom implements Module {
	...
	*/
	function init_pane() {
		...
		/*
		** Loving stolen directly from Intranet 1
		*/
		if (isSet($_POST['firstpdteacher'])) {
			...
			//fixed I1 bug: user $I2_USER->username instead of $_POST
        $msg = "Person's Name: {$I2_USER->fullname_comma}\nAttending Prom: $going\nFirst period teacher: {$_POST['firstpdteacher']}\nFirst period room: {$_POST['
firstpdroom']}\n\nIf the next stuff is blank that means that they will be dating someone from the class of 2006.\nOut of Class Date Name: {$_POST['datename
']}\nOut of Class Date School (or year @ TJ): {$_POST['dateschool']}\n\nNOTE: This message was generated by Intranet 2 and may not adhere to the format of previous messages.  For example, the contained name is automatically ascertained instead of allowing the user to enter it (or someone else's name).";
        $email = 'tjprom2006@gmail.com';
