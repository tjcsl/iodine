<?php
/**
* Just contains the definition for the class {@link PodcastQuestion}.
...
/**
* The class that keeps track of one question in a podcast
* @package modules
* @subpackage Podcasts
*/
class PodcastQuestion {
	...
	public function __get($var) {
		...
		default:
			throw new I2Exception("Invalid variable $var attempted to be accessed in PodcastQuestion");
		}
	}

	/**
	 * Constructs a new PodcastQuestion with the given pid and qid.
	 *
	...
	public function __construct($pid, $qid) {
		...
		//	throw new I2Exception("Invalid PodcastQuestion attempted to be created with qid $qid");
		//}

		$info = $I2_SQL->query('SELECT maxvotes, question, answertype FROM podcast_questions WHERE pid=%d AND qid=%d', $pid, $qid)->fetch_array(Result::ASSOC);
	...
	* 
	* This inserts a question into the database; it does NOT add it to any pre-existing {@link Podcast}
	* objects. Usually the {@link Podcast} add_question method, which calls this, should be used instead.
	*
	* @param int $pid The ID number of the podcast
	...
	*/
	public static function new_question($pid, $question, $answertype, $maxvotes, $qid=NULL) {
		...
		if ($qid == NULL) {
			$qid = $I2_SQL->query('SELECT MAX(qid) FROM podcast_questions WHERE pid=%d',$pid)->fetch_single_value();
			if ($qid === NULL)
				$qid = 0;
			else
				$qid += 1;
		...
		$I2_SQL->query('INSERT INTO podcast_questions SET pid=%d, qid=%d, question=%s, answertype=%s, maxvotes=%d', $pid, $qid, $question, $answertype, $maxvotes);
		return new PodcastQuestion($pid,$qid);
	}

	public function edit_question($text, $answertype, $maxvotes) {
		global $I2_SQL;

		$I2_SQL->query('UPDATE podcast_questions SET question=%s, answertype=%s, maxvotes=%d WHERE pid=%d AND qid=%d', $text, $answertype, $maxvotes, $this->mypid, $this->myqid);
	}

	public function add_answer($text, $aid=NULL) {
		global $I2_SQL;
		if ($aid === NULL) {
			$aid = $I2_SQL->query('SELECT MAX(aid) FROM podcast_responses WHERE pid=%d AND qid=%d',$this->mypid,$this->myqid)->fetch_single_value();
			if ($aid === NULL)
				$aid = 0;
			else
				$aid += 1;
		...
		$I2_SQL->query('INSERT INTO podcast_responses SET pid=%d, qid=%d, aid=%d, answer=%s',$this->mypid,$this->myqid,$aid,$text);
	...
	public function edit_answer($text, $aid) {
		...
		$I2_SQL->query('UPDATE podcast_responses SET answer=%s WHERE pid=%d AND qid=%d AND aid=%d',$text,$this->mypid,$this->myqid,$aid);
		$this->myanswers[$aid] = $text;
	}
	...
	public function delete_answer($aid) {
		...
		$I2_SQL->query('DELETE FROM podcast_responses WHERE pid=%d AND qid=%d AND aid=%d',$this->mypid,$this->myqid,$aid);
	...
	public function user_voted_for($aid) {
		...
		return $I2_SQL->query('SELECT COUNT(*) FROM podcast_votes WHERE pid=%d AND qid=%d AND aid=%d AND uid=%d',$this->mypid,$this->myqid,$aid,$I2_USER->uid)->fetch_single_value() > 0;
	}
	...
	public function get_response($uid = NULL) {
		...
		if ($this->myanswertype == 'free_response' || $this->myanswertype == 'short_response')
			return $I2_SQL->query('SELECT written FROM podcast_votes WHERE pid=%d AND qid=%d AND uid=%d',$this->mypid,$this->myqid,$uid)->fetch_single_value();
		else if ($this->myanswertype == 'standard')
			return $I2_SQL->query('SELECT aid FROM podcast_votes WHERE pid=%d AND qid=%d AND uid=%d',$this->mypid,$this->myqid,$uid)->fetch_single_value();
		else if ($this->myanswertype == 'approval')
			return $I2_SQL->query('SELECT aid FROM podcast_votes WHERE pid=%d AND qid=%d AND uid=%d ORDER BY aid DESC',$this->mypid,$this->myqid,$uid)->fetch_all_single_values();
	...
	public function delete_vote($uid = NULL) {
		...
		if ($uid === NULL)
			$uid = $I2_USER->uid;
		$exists = $I2_SQL->query('SELECT COUNT(*) FROM podcast_votes WHERE pid=%d AND qid=%d AND uid=%d',
			$this->mypid,$this->myqid,$uid)->fetch_single_value() > 0;
		if ($exists)
			$I2_SQL->query('DELETE FROM podcast_votes WHERE pid=%d AND qid=%d AND uid=%d',
	...
	public function vote($post, $uid = NULL) {
		...
		case 'free_response':
			...
			$I2_SQL->query('INSERT INTO podcast_votes SET pid=%d,qid=%d,uid=%d,written=%s',
		...
		case 'standard':
			if($post >= 0) {
				$I2_SQL->query('INSERT INTO podcast_votes SET pid=%d,qid=%d,uid=%d,aid=%d',
					$this->mypid, $this->myqid, $uid, $post);
			}
			break;
		case 'approval':
	...
	public function num_who_vote($aid, $grade, $gender) {
		...
		case 'standard':
		case 'approval':
			return $I2_SQL->query('SELECT COUNT(*) FROM podcast_votes '
		...
		case 'split_approval':
			...
			$res = $I2_SQL->query('SELECT uid FROM podcast_votes WHERE pid=%d AND qid=%d AND aid=%d '.
	...
	public function num_who_voted() {
		...
		return $I2_SQL->query('SELECT COUNT(DISTINCT uid) FROM podcast_votes WHERE pid=%d AND qid=%d',$this->mypid,$this->myqid)->fetch_single_value();
	}
